...
class UsedListContainer
{
   public:
	 ...
	 for (unsigned i = 0; i < m_size; i++)
	 {
	    unsigned rot_val = (unsigned)m_list[i];
...
template <class Key, class T> class UsedList
{
   public:
	 ...
	 std::unordered_map<Key, std::vector<UsedListContainer> >::const_iterator it = m_hash.find(container.Hash());
	 m_accesses += 1;
	 if (it == m_hash.end())
	    return false;
	 ...
	 std::unordered_map<Key, std::vector<UsedListContainer> >::iterator it = m_hash.find(container.Hash());
	 m_accesses += 1;
	 if (it == m_hash.end())
	 {
	    m_entries += 1;
