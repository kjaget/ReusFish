...
void BarrelCactus::GetYield(std::vector<Space> &spaces, unsigned loc, Yield &yield, unsigned m_tech_adder, unsigned m_wealth_limit) const
{
   ...
   if (wealth_yield[loc] >= m_wealth_limit)
...
{ 
   ...
   for (unsigned i = 0; i < spaces.size(); i++)
   {
      ...
      {
	 Yield range_yield;
	 spaces[i].m_source->GetYield(spaces, i, range_yield);
	 for (unsigned j = std::max<int>(0, (int)loc - range_yield.m_range); (i <= loc + range_yield.m_range) && (i < spaces.size()); i+= 1)
