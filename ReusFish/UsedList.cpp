...
UsedListContainer::UsedListContainer(const Landscape &landscape)
{
...
UsedListContainer::UsedListContainer(const UsedListContainer &orig)
{
	m_size = orig.m_size;
	m_list = new const Source *[m_size];
...
UsedListContainer::~UsedListContainer()
{
	delete [] m_list;
	delete [] m_blist;
}
bool UsedListContainer::operator==(const UsedListContainer &rhs) const
{
...
void UsedListContainer::SetHash(void)
{
	m_hash = 0;
	for (unsigned i = 0; i < m_size; i++)
	{
...
size_t UsedListContainer::Hash(void) const
{ 
	return m_hash;
}
...
template <class Key, class T>
bool UsedList<Key, T>::Exists(const T &entry)
{
...
template <class Key, class T>
bool UsedList<Key, T>::Insert(const T &entry)
{
	UsedListContainer container(entry);
	Key key = container.Hash();

	typename std::unordered_map<Key, std::vector<UsedListContainer> >::iterator it = m_hash.find(key);
	m_accesses += 1;
	if (it == m_hash.end())
	{
		m_entries += 1;
		m_hash[key] = std::vector<UsedListContainer>(1, container);
		return true;
	}
	//std::cout << "=====================================" << std::endl;
	///entry.PrintAll();
...
template <class Key, class T>
void UsedList<Key, T>::Print(void) const
{
...
template class UsedList<size_t, Landscape>;
